...
func TestFormatRuntimeGaugesUsesGaugesAndCounterDeltas(t *testing.T) {
	...
	current := statsSnapshot{
		...
		wireResponseDepth:    5,
		wireResponseCap:      100,
		wireSourceSchedule:   true,
		wireAnnounceDepth:    12,
		wireAnnounceCap:      75,
		wireAnnounceIn:       40,
		wireAnnounceDrop:     3,
		wireLookupDepth:      4,
	...
	for _, want := range []string{
		...
		"wire_src_sched=true", "wire_ann_depth=12", "wire_ann_cap=75", "wire_ann_in=40", "wire_ann_drop=3",
		"wire_gp_depth=4", "wire_gp_cap=25", "wire_gp_in=20", "wire_gp_drop=2",
...
func TestAddWireWorkerStatsExposesTargetActiveCeilingAndPinned(t *testing.T) {
	...
	want := map[string]uint64{
		...
		"wire_source_scheduler_enabled": 1,
		"wire_announce_queue_depth":     12,
		"wire_announce_queue_capacity":  75,
		"wire_announce_queue_admitted":  40,
		"wire_announce_queue_dropped":   3,
		"wire_lookup_queue_depth":       4,
		"wire_lookup_queue_capacity":    25,
		"wire_lookup_queue_admitted":    20,
		"wire_lookup_queue_dropped":     2,
	}
	for key, value := range want {
		if out[key] != value {
			t.Errorf("worker stat %s = %d, want %d", key, out[key], value)
