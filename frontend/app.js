        ...
        poll: function () {
            ...
            Promise.allSettled([s.fetchJson('/api/v1/stats'), s.fetchJson('/health')]).then(function (results) {
                ...
                if (results[0].status === 'fulfilled') {
                    var nextStats = results[0].value, metadataCount = Number(nextStats.dedupFilterSize) || 0;
                    s.st = nextStats;
                    if (s.previousMetadata && metadataCount < s.previousMetadata.value) s.metadataRate = null;
                    if (!s.previousMetadata || metadataCount !== s.previousMetadata.value) {
                        if (s.previousMetadata && metadataCount >= s.previousMetadata.value) s.metadataRate = (metadataCount - s.previousMetadata.value) / ((now - s.previousMetadata.at) / 1000);
                        s.previousMetadata = { value: metadataCount, at: now };
