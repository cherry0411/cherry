...
// ---- Components ----
var HomePage = {
    template: '<div class="hero"><div class="hero-logo">🍒</div><h1>{{ T("hero_title") }}</h1><div class="hero-search"><input class="search-input" :placeholder="T(\'hero_placeholder\')" v-model="q" @keydown.enter="goSearch" ref="inp" /><button class="search-btn" @click="goSearch">{{ T("search_btn") }}</button></div><div class="hero-hash"><input placeholder="info_hash..." v-model="hq" @keydown.enter="lookupHash" /><button class="hash-btn" @click="lookupHash">{{ T("lookup") }}</button></div><div v-if="history.length" class="search-history"><span v-for="h in history" class="history-chip" @click="goHistory(h)">{{ h }}</span></div><section class="status-board" aria-live="polite"><div class="status-card status-state"><span class="status-label">Status</span><strong><i class="status-dot" :class="{active:online,crawling:isCrawling}"></i>{{ stateText }}</strong><small v-if="lastUpdated">{{ T("last_update") }} {{ lastUpdated }}</small></div><div class="status-card"><span class="status-label">{{ T("stat_total") }}</span><strong>{{ fmtNumber(st.pgCatalogEstimate) }}</strong><small>+{{ fmtNumber(st.todayNew) }} {{ T("stat_today") }}</small></div><div class="status-card"><span class="status-label">{{ T("metadata_rate") }}</span><strong>{{ fmtRate(metadataRate) }}</strong><small>{{ metadataFunnelText }}</small></div><div class="status-card"><span class="status-label">{{ T("heat_records") }}</span><strong>{{ fmtNumber(acceptedHeat) }}</strong><small>{{ T("heat_rate") }} {{ fmtRate(heatRate) }}/s</small></div><div class="status-card"><span class="status-label">{{ T("rolling_heat") }}</span><strong>{{ capacityPercent }}</strong><small>{{ capacityText }}</small></div></section><div class="footer">Ctrl+K to search</div></div>',
    ...
    computed: {
        ...
        durableIngest: function () { return this.st.durableIngest || {}; },
        metadataFunnelText: function () { return T('metadata_committed') + ' ' + fmtNumber(this.durableIngest.metadataCommitted) + ' · ' + T('durable_delivered') + ' ' + fmtNumber(this.durableIngest.deliveredRecords) + ' · ' + T('durable_accepted') + ' ' + fmtNumber(this.durableIngest.acceptedRecords) + ' · ' + T('durable_duplicate') + ' ' + fmtNumber(this.durableIngest.duplicateRecords); },
        ...
        poll: function () {
            ...
            Promise.allSettled([s.fetchJson('/api/v1/stats'), s.fetchJson('/health')]).then(function (results) {
                ...
                if (results[0].status === 'fulfilled') {
                    var nextStats = results[0].value, durable = nextStats.durableIngest || {}, metadataCount = Number(durable.metadataCommitted) || 0, metadataAt = Date.parse(nextStats.serverTime) || now, counterBaseline = durable.countersStartedAt || '';
                    s.st = nextStats;
                    if (s.previousMetadata && (metadataCount < s.previousMetadata.value || counterBaseline !== s.previousMetadata.baseline)) { s.metadataRate = null; s.previousMetadata = null; }
                    if (!s.previousMetadata) s.previousMetadata = { value: metadataCount, at: metadataAt, baseline: counterBaseline };
                    else if (metadataAt > s.previousMetadata.at) {
                        s.metadataRate = (metadataCount - s.previousMetadata.value) / ((metadataAt - s.previousMetadata.at) / 1000);
                        s.previousMetadata = { value: metadataCount, at: metadataAt, baseline: counterBaseline };
                    }
                }
