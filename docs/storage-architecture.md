...
3. 只保留 `24h/3d/7d/15d` 四个实际可选的 sortable heat 字段；发现时间是独立 filter/sort，不能与
   `heatWindow` 混为一个参数。
...
- actor 是 crawler 内存中用两区共享 master secret 计算的 stable rolling HMAC64；
  原始 IP、端口和 node ID 不出 crawler、不落 spool/PG/备份。IPv4 用完整地址，IPv6
  用 /64；port/node churn 不能刷同一资源的热度，NAT 合并带来的保守低估必须在 UI
  语义中承认。stable token 只进入 storage host 的 disposable rolling SQLite；daily SQLite
  写入前再用 storage-only 日密钥重 HMAC，PG/Meili/备份都不保存 stable actor。
...
- `24h` 是上一完整小时为截止点的 24 个 UTC 小时 exact unique actor；`3/7/15d` 是
  daily actor-day 之和，均不冒充唯一用户/peer。
...
- Meili 只投影已 sealed 日历日。若上一成功日为 `D-1`，可能变化的 ID 严格来自
  `D ∪ D-3 ∪ D-7 ∪ D-15`；只为这些 ID 扫描当前 15 帧计算三个 daily 绝对值并
  partial PUT，三元组未变化则跳过。24h 由 rolling SQLite 的 dirty hash 增量投影。
  漏投多天必须逐日补齐；首次/新索引重建从全部 base docs 和当前 15 日 union 开始。
- daily frame 的 GC 由 Meili `projected_through` 水位控制，不能在投影故障时机械限制
  为 16 天。metadata outbox 与 heat 都使用 partial PUT；PG catalog join 证明 ID 存在
