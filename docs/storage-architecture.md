...
| heat | `peer_count` 累计值 | 带 receipt 的唯一 actor batch + 1/7/15/30 天窗口 |
...
- payload 使用版本号、varint、UTF-8 和排序路径前缀压缩；同一编码同时容纳 full 文件和
  summary 的代表文件/扩展统计，但不保存 retention 标记。
...
| `summary` | title、scalars、扩展名/大小汇总、至多 K 个代表文件供详情显示 | 如 `files > 2000` 或路径总字节超限 | 只索引 title |
